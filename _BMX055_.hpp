...
void BMX055_Read()
{
  ...
  for (int i = 0; i < 6; i++)
  {
    ...
    if (Wire.available() == 1)
    {
      data_tmp[i] = Wire.read();
    }
  }

  // Convert the data to 12-bits
  int xAccl = ((data_tmp[1] * 256) + (data_tmp[0] & 0xF0)) / 16;
  if (xAccl > 2047)
  {
    xAccl -= 4096;
  }
  int yAccl = ((data_tmp[3] * 256) + (data_tmp[2] & 0xF0)) / 16;
  if (yAccl > 2047)
  {
    yAccl -= 4096;
  }
  int zAccl = ((data_tmp[5] * 256) + (data_tmp[4] & 0xF0)) / 16;
  if (zAccl > 2047)
  {
    zAccl -= 4096;
  }
  ...
  // Convert the data
  int xGyro = (data_tmp[1] * 256) + data_tmp[0];
  if (xGyro > 32767)
  {
    xGyro -= 65536;
  }
  int yGyro = (data_tmp[3] * 256) + data_tmp[2];
  if (yGyro > 32767)
  {
    yGyro -= 65536;
  }
  int zGyro = (data_tmp[5] * 256) + data_tmp[4];
  if (zGyro > 32767)
  {
    zGyro -= 65536;
  }
  ...
  // Convert the data
  int xMag = ((data_tmp[1] * 256) + (data_tmp[0] & 0xF8)) / 8;
  if (xMag > 4095)
  {
    xMag -= 8192;
  }
  int yMag = ((data_tmp[3] * 256) + (data_tmp[2] & 0xF8)) / 8;
  if (yMag > 4095)
  {
    yMag -= 8192;
  }
  int zMag = ((data_tmp[5] * 256) + (data_tmp[4] & 0xFE)) / 2;
  if (zMag > 16383)
  {
    zMag -= 32768;
  }
